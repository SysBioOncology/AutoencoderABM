...
def vis_tumoroid(pca_results, y_train, explained_variance,
    ...
    fig, ax = plt.subplots(figsize = (6, 5))
    pca_fig = ax.scatter(pca_results[y_train[:,1] == 72,0], pca_results[y_train[:,1] == 72,1],
                         c = y_train[y_train[:,1]==72,0])
    cb = fig.colorbar(pca_fig, ax=ax)
    cb.set_label('z stacks')
    ax.set_xlabel('PC1');
    ax.set_ylabel('PC2');
    ...
    title_variance = round(title_variance, 4)
    ax.set_xlim(14.5, 16.5)
    ax.set_title(f"Explained variance: {title_variance:.4f}, time = 72h")

    fig.savefig(f"{output_folder}/train_t=72_z_encoded_nSamples={samples}_epochs={epochs}_batch={batch}.png")
